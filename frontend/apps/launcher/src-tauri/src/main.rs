...
fn finalize_pending_update() -> Result<(), String> {
    ...
    if marker_path.exists() {
        ...
        let content = fs::read_to_string(&marker_path).map_err(|e| format!("读取标记文件失败: {}", e))?;
        let marker: UpdateMarker = serde_json::from_str(&content).map_err(|e| format!("解析标记文件失败: {}", e))?;
        ...
        println!("[Main] local_versions.json 已更新至版本: {}", marker.version);
