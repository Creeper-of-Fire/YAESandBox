...
*   **工作流 (Workflow):** 一个由多个“步骤”组成的配置。它不再是简单的线性列表，而是一个**数据流图**的表示，定义了从初始输入到最终输出的完整转换逻辑。
*   **步骤 (Step):** 工作流的基本执行单元，类似于一个“函数”或“节点”。每个步骤都是符文化、可配置的，封装了一部分具体的处理逻辑（如生成提示词、调用AI、处理文本等）。
*   **符文 (Rune):** 构成步骤的最小功能单元。每个符文执行一个原子性的任务，并**声明**它需要消费什么**输入变量**（`Consumes`）以及会生产什么**输出变量**（`Produces`）。
*   **变量与作用域 (Variables & Scope):**
    *   **全局变量池:** 整个工作流共享一个全局变量池。这是数据在**步骤之间**传递的唯一渠道。
    *   **步骤局部变量池:** 每个步骤在执行时，都有一个临时的、私有的变量池。符文产生的变量首先进入这里。
    *   **输出映射 (`OutputMappings`):** 步骤配置的核心。它像一个“出口防火墙”，明确定义了哪些**局部变量**可以被“发布”到**全局变量池**中，以及它们在全局中的名字。任何未被映射的局部变量都会在步骤执行完毕后被销GL销。
*   **依赖与并行 (Dependencies & Parallelism):**
    *   引擎**不依赖**于步骤在配置文件中的物理顺序。
    *   它通过分析全局变量的“生产-消费”关系来**自动推断**步骤间的依赖。
    *   如果步骤A的输出是步骤B的输入，则B依赖于A。
    *   没有相互依赖关系的步骤将被**自动并行执行**。
...
2.  **编译 (Compile):**
    *   引擎首先对工作流配置进行**依赖分析**。
    *   它检查每个步骤的`GlobalConsumers`（来自符文的`Consumes`）和`GlobalProducers`（来自步骤的`OutputMappings.Keys`）。
    *   基于此构建一个有向无环图（DAG），确定所有步骤间的依赖关系。
    *   这个阶段会检测出无法执行的**循环依赖**。
3.  **调度与执行 (Schedule & Execute):**
    *   引擎根据编译好的DAG，采用拓扑排序的方式进行调度。
    *   **并行执行批次:** 将所有无未满足依赖的步骤作为一个“批次”，使用 `Task.WhenAll` 并行执行。
    *   **步骤执行:** 每个步骤执行时，会先从全局变量池拉取所需输入到其局部变量池，然后按顺序执行其内部符文。符文间通过局部变量池传递数据。
    *   **结果发布:** 步骤执行完毕后，根据其`OutputMappings`将结果以线程安全的方式写回全局变量池。
    *   **迭代:** 当一个批次执行完毕后，引擎会检查哪些新的步骤因为依赖得到满足而变得可执行，并将它们作为下一个并行批次，如此循环直至所有步骤完成。
4.  **完成:** 所有步骤执行完毕。引擎将最终的全局变量池返回给调用者，或者根据“终结节点”的配置组装成 `WorkflowExecutionResult`。
5.  **清理:** 引擎实例销毁，所有运行时上下文（包括全局变量池）被丢弃。

## 步骤详解 (Step Details)

一个“步骤 (Step)”是工作流的基本构建块和执行单元。它代表工作流中的一个处理阶段。步骤是符文化的，可以独立配置和存储，并被不同的工作流引用。
