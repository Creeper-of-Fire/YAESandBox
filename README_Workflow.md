...
*   **步骤 (Step):** 工作流的基本执行单元，类似于一个“函数”或“节点”。每个步骤都是模块化、可配置的，封装了一部分具体的处理逻辑（如生成提示词、调用AI、处理文本等）。
*   **模块 (Module):** 构成步骤的最小功能单元。每个模块执行一个原子性的任务，并**声明**它需要消费什么**输入变量**（`Consumes`）以及会生产什么**输出变量**（`Produces`）。
*   **变量与作用域 (Variables & Scope):**
    *   **全局变量池:** 整个工作流共享一个全局变量池。这是数据在**步骤之间**传递的唯一渠道。
    *   **步骤局部变量池:** 每个步骤在执行时，都有一个临时的、私有的变量池。模块产生的变量首先进入这里。
...
2.  **编译 (Compile):**
    *   引擎首先对工作流配置进行**依赖分析**。
    *   它检查每个步骤的`GlobalConsumers`（来自模块的`Consumes`）和`GlobalProducers`（来自步骤的`OutputMappings.Keys`）。
...
3.  **调度与执行 (Schedule & Execute):**
    ...
    *   **步骤执行:** 每个步骤执行时，会先从全局变量池拉取所需输入到其局部变量池，然后按顺序执行其内部模块。模块间通过局部变量池传递数据。
...
*   **声明优于指令:** 用户只需声明“做什么”（模块和变量关系），而无需关心“怎么做”（执行顺序和并行）。
