...
public class BlocksControllerTests
{
    ...
    public BlocksControllerTests()
    {
        ...
        _mockWritService = new Mock<IBlockWritService>(); // 初始化 mock
        _controller = new BlocksController(_mockWritService.Object, _mockReadService.Object);
    }
    ...
    public async Task GetBlocks_应返回所有Block详情的OkResult()
    {
        ...
        _mockReadService.Setup(s => s.GetAllBlockDetailsAsync()).ReturnsAsync(blockDetails);
        ...
        _mockReadService.Verify(s => s.GetAllBlockDetailsAsync(), Times.Once);
    }
    ...
    public async Task GetBlockDetail_当Block存在时_应返回Block详情的OkResult()
    {
        ...
        _mockReadService.Setup(s => s.GetBlockDetailDtoAsync(blockId)).ReturnsAsync(blockDetail);

        // Act
        var result = await _controller.GetBlockDetail(blockId);

        // Assert
        var okResult = result.Should().BeOfType<OkObjectResult>().Subject;
        okResult.Value.Should().BeSameAs(blockDetail); // 可以是 BeSameAs 或 BeEquivalentTo
        _mockReadService.Verify(s => s.GetBlockDetailDtoAsync(blockId), Times.Once);
    }
    ...
    public async Task GetBlockDetail_当Block不存在时_应返回NotFoundResult()
    {
        ...
        _mockReadService.Setup(s => s.GetBlockDetailDtoAsync(blockId)).ReturnsAsync((BlockDetailDto?)null); // 模拟返回 null
    ...
    public async Task GetTopology_当服务成功返回拓扑时_应返回包含拓扑的OkResult()
    {
        ...
        // 模拟服务返回 JSON 节点对象
        _mockReadService.Setup(s => s.GetBlockTopologyJsonAsync()).ReturnsAsync(topologyNode);

        // Act
        var result = await _controller.GetTopology();
        ...
        _mockReadService.Verify(s => s.GetBlockTopologyJsonAsync(), Times.Once);
    }
    ...
    {
        // Arrange
        _mockReadService.Setup(s => s.GetBlockTopologyJsonAsync()).ReturnsAsync((BlockTopologyExporter.JsonBlockNode?)null);
    ...
    {
        // Arrange
        _mockReadService.Setup(s => s.GetBlockTopologyJsonAsync()).ThrowsAsync(new Exception("模拟服务层错误"));
