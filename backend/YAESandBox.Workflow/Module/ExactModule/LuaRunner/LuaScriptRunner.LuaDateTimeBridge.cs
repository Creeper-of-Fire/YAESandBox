...
internal partial class LuaScriptRunner
{
    ...
    private class LuaDateTimeBridge(LuaLogBridge logger)
    {
        ...
        public LuaDateTimeObject utcnow() => new(DateTimeOffset.UtcNow, this.Logger);
        public LuaDateTimeObject now() => new(DateTimeOffset.Now, this.Logger);
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(format))
                {
                    ...
                    if (DateTimeOffset.TryParse(dateString, CultureInfo.InvariantCulture, DateTimeStyles.None, out result))
                        return new LuaDateTimeObject(result, this.Logger);
                ...
                return DateTimeOffset.TryParseExact(
                    ...
                    DateTimeStyles.None, 
                    out result) 
                    ? new LuaDateTimeObject(result, this.Logger) 
                    : null;
            }
            catch (Exception ex)
            {
                this.Logger.error($"datetime.parse 失败: {ex.Message}");
                return null;
            }
        }
    }
