...
/// </summary>
internal class ModuleConfigConverter : JsonConverter<IModuleConfig>
{
    // 使用 nameof 获取规范的属性名称，以增强代码健壮性。
    // 这是我们期望在 JSON 中查找的属性名，比较时会忽略大小写。
    private const string ExpectedModuleTypePropertyName = nameof(IModuleConfig.ModuleType);

    /// <inheritdoc />
    public override bool CanConvert(Type typeToConvert)
    {
        // 此转换器仅用于 IModuleConfig 接口。
        return typeToConvert == typeof(IModuleConfig);
    }

    /// <inheritdoc />
    public override IModuleConfig? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        if (reader.TokenType != JsonTokenType.StartObject)
        {
            // 预期的 JSON 对象起始符号 '{' 未找到。
            throw new JsonException($"反序列化 IModuleConfig 失败：预期的 JSON 对象起始符号 '{{' 未找到，当前 Token 类型为 {reader.TokenType}。");
        }
        ...
        if (!moduleTypePropertyFound)
        {
            ...
            throw new JsonException($"反序列化 IModuleConfig 失败：无法在JSON对象中找到属性 '{ExpectedModuleTypePropertyName}' (忽略大小写)，该属性用于确定具体的模块配置类型。");
        }

        if (string.IsNullOrWhiteSpace(moduleTypeNameFromInput))
        {
            // ModuleType 属性的值为空或空白。
            throw new JsonException($"反序列化 IModuleConfig 失败：属性 '{actualPropertyNameFound}' 的值不能为空或仅包含空白字符。");
        }
        ...
        var moduleConfig = jsonObject.Deserialize(concreteType, options) as IModuleConfig;
    ...
    /// <inheritdoc />
    public override void Write(Utf8JsonWriter writer, IModuleConfig value, JsonSerializerOptions options)
    {
        // 对于序列化，System.Text.Json 在传递实际类型（value.GetType()）时能正确处理。
        // 它会序列化具体类型的所有属性，包括 ModuleType 属性。
        // ModuleType 属性的名称将根据其在具体类中的定义（通常是 "ModuleType"，因为 nameof(IModuleConfig.ModuleType) 是 "ModuleType"）
