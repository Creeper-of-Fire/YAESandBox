...
public record DataWithSchemaDto<T> where T : notnull
{
    ...
    public static Result<DataWithSchemaDto<T>> Create(T data)
    {
        ...
        {
            return JsonError.Error(schemaJson, $"为类型 {type}生成的 Schema 不是有效的 JSON 格式。错误: {jsonEx.Message}");
        }
        catch (Exception ex)
        {
            return JsonError.Error(schemaJson, $"为类型 '{type}' 生成配置模板时发生内部错误: {ex.Message}");
        }
