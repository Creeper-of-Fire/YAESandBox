...
public static class ResultToActionExtensionsAsync
{
    ...
    public static async Task<ActionResult<Dictionary<TKey, SingleItemResultDto<TValue>>>> ToActionResultAsync<TKey, TValue>(
        this Task<DictionaryResult<TKey, TValue>> taskDictionaryResult) where TKey : notnull
    {
    ...
    /// <returns>一个表示异步转换操作的 Task，其最终结果为相应的 ActionResult。</returns>
    public static async Task<ActionResult<List<SingleItemResultDto<T>>>> ToActionResultAsync<T>(
