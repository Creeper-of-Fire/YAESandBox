...
public class BlockWritServiceTests
{
    ...
    public BlockWritServiceTests()
    {
        ...
        this._blockManagerMock = Substitute.For<IBlockManager>();
        this._sut = new BlockWritService(this._notifierServiceMock, this._blockManagerMock);
    }
    ...
    public async Task EnqueueOrExecuteAtomicOperationsAsync_当Block为Idle时_应执行并通知()
    {
        ...
        this._blockManagerMock.EnqueueOrExecuteAtomicOperationsAsync(blockId, operations)
        ...
        // Act
        var result = await this._sut.EnqueueOrExecuteAtomicOperationsAsync(blockId, operations);

        // Assert
        result.Should().Be(AtomicExecutionResult.Executed);
        // 验证 BlockManager 被调用
        await this._blockManagerMock.Received(1).EnqueueOrExecuteAtomicOperationsAsync(blockId, operations);
        // 验证 Notifier 被调用
        await this._notifierServiceMock.Received(1).NotifyBlockStatusUpdateAsync(blockId, BlockStatusCode.Idle);
        await this._notifierServiceMock.Received(1).NotifyStateUpdateAsync(blockId, Arg.Is<IEnumerable<string>>(ids => ids.Count() == 2)); // 验证实体 ID 列表
    }
    ...
    public async Task EnqueueOrExecuteAtomicOperationsAsync_当Block为Loading时_应排队并通知()
    {
        ...
        await this._blockManagerMock.Received(1).EnqueueOrExecuteAtomicOperationsAsync(blockId, operations);
        await this._notifierServiceMock.Received(1).NotifyBlockStatusUpdateAsync(blockId, BlockStatusCode.Loading);
        await this._notifierServiceMock.Received(1).NotifyStateUpdateAsync(blockId, Arg.Any<IEnumerable<string>>());
    }
    ...
    public async Task EnqueueOrExecuteAtomicOperationsAsync_当Block为Conflict时_应返回Conflict且不通知State()
    {
        ...
        await this._notifierServiceMock.DidNotReceive().NotifyStateUpdateAsync(Arg.Any<string>(), Arg.Any<IEnumerable<string>>());
    ...
    public async Task EnqueueOrExecuteAtomicOperationsAsync_当Block不存在时_应返回NotFound()
    {
        ...
        await this._blockManagerMock.Received(1).EnqueueOrExecuteAtomicOperationsAsync(blockId, operations);
        await this._notifierServiceMock.DidNotReceive().NotifyBlockStatusUpdateAsync(Arg.Any<string>(), Arg.Any<BlockStatusCode>());
        await this._notifierServiceMock.DidNotReceive().NotifyStateUpdateAsync(Arg.Any<string>(), Arg.Any<IEnumerable<string>>());
    }
    ...
    public async Task UpdateBlockGameStateAsync_当Block存在时_应调用Manager并返回Success()
    {
        ...
        this._blockManagerMock.UpdateBlockGameStateAsync(blockId, settings)
                         .Returns(Task.FromResult(UpdateResult.Success));

        // Act
        var result = await this._sut.UpdateBlockGameStateAsync(blockId, settings);

        // Assert
        result.Should().Be(UpdateResult.Success);
        await this._blockManagerMock.Received(1).UpdateBlockGameStateAsync(blockId, settings);
    }
    ...
    public async Task ApplyResolvedCommandsAsync_当BlockManager成功时_应通知更新()
    {
        ...
        this._blockManagerMock.ApplyResolvedCommandsAsync(blockId, resolvedCommands)
        ...
        // Act
        await this._sut.ApplyResolvedCommandsAsync(blockId, resolvedCommands);

        // Assert
        await this._blockManagerMock.Received(1).ApplyResolvedCommandsAsync(blockId, resolvedCommands);
        await this._notifierServiceMock.Received(1).NotifyBlockStatusUpdateAsync(blockId, BlockStatusCode.Idle);
        await this._notifierServiceMock.Received(1).NotifyStateUpdateAsync(blockId, Arg.Is<IEnumerable<string>>(ids => ids.Count() == 2));
    }
    ...
    public async Task ApplyResolvedCommandsAsync_当BlockManager返回Error时_应通知更新但标记为Error()
    {
        ...
        await this._blockManagerMock.Received(1).ApplyResolvedCommandsAsync(blockId, resolvedCommands);
        // 仍然需要通知状态变化为 Error
        await this._notifierServiceMock.Received(1).NotifyBlockStatusUpdateAsync(blockId, BlockStatusCode.Error);
    ...
    public async Task CreateChildBlockForWorkflowAsync_当Manager成功时_应返回Block并通知()
    {
        ...
        this._blockManagerMock.CreateChildBlock_Async(parentBlockId, triggerParams)
                         .Returns(Task.FromResult<LoadingBlockStatus?>(newBlock));

        // Act
        var result = await this._sut.CreateChildBlockAsync(parentBlockId, triggerParams);

        // Assert
        result.Should().NotBeNull();
        result.Should().BeSameAs(newBlock);
        await this._blockManagerMock.Received(1).CreateChildBlock_Async(parentBlockId, triggerParams);
        await this._notifierServiceMock.Received(1).NotifyBlockStatusUpdateAsync(newBlockId, BlockStatusCode.Loading);
    ...
    public async Task HandleWorkflowCompletionAsync_当成功且无冲突时_应调用Manager()
    {
        ...
        this._blockManagerMock.HandleWorkflowCompletionAsync(blockId, true, rawText, commands, outputVars)
        ...
        // Act
        await this._sut.HandleWorkflowCompletionAsync(blockId, requestId, true, rawText, commands, outputVars);

        // Assert
        await this._blockManagerMock.Received(1).HandleWorkflowCompletionAsync(blockId, true, rawText, commands, outputVars);
        // 在这种情况下，writ service 不直接通知，manager 内部处理状态转换后由其他流程通知（或writ service 的其他方法通知）
        await this._notifierServiceMock.DidNotReceive().NotifyConflictDetectedAsync(Arg.Any<ConflictDetectedDto>());
    }
    ...
    public async Task HandleWorkflowCompletionAsync_当成功但有冲突时_应调用Manager并通知冲突()
    {
        ...
        this._blockManagerMock.HandleWorkflowCompletionAsync(blockId, true, rawText, aiCommands, outputVars) // 假设参数只传 AI 指令
                         .Returns(Task.FromResult<OneOf<(OneOf<IdleBlockStatus, ErrorBlockStatus> blockStatus, List<OperationResult> results), ConflictBlockStatus, ErrorBlockStatus>?>(managerResult));


        // Act
        await this._sut.HandleWorkflowCompletionAsync(blockId, requestId, true, rawText, aiCommands, outputVars);

        // Assert
        await this._blockManagerMock.Received(1).HandleWorkflowCompletionAsync(blockId, true, rawText, aiCommands, outputVars);
        // 验证冲突通知被发送
        await this._notifierServiceMock.Received(1).NotifyConflictDetectedAsync(Arg.Is<ConflictDetectedDto>(dto =>
            dto.BlockId == blockId &&
    ...
    public async Task HandleWorkflowCompletionAsync_当失败时_应调用Manager()
    {
        ...
        // 模拟 Manager 返回 Error 状态 (T2 of the outer OneOf)
         var managerResult = OneOf<(OneOf<IdleBlockStatus, ErrorBlockStatus> blockStatus, List<OperationResult> results), ConflictBlockStatus, ErrorBlockStatus>
             .FromT2(errorStatus);
         this._blockManagerMock.HandleWorkflowCompletionAsync(blockId, false, rawText, commands, outputVars)
                          .Returns(Task.FromResult<OneOf<(OneOf<IdleBlockStatus, ErrorBlockStatus> blockStatus, List<OperationResult> results), ConflictBlockStatus, ErrorBlockStatus>?>(managerResult));

        // Act
        await this._sut.HandleWorkflowCompletionAsync(blockId, requestId, false, rawText, commands, outputVars);

        // Assert
        await this._blockManagerMock.Received(1).HandleWorkflowCompletionAsync(blockId, false, rawText, commands, outputVars);
        await this._notifierServiceMock.DidNotReceive().NotifyConflictDetectedAsync(Arg.Any<ConflictDetectedDto>());
    }
