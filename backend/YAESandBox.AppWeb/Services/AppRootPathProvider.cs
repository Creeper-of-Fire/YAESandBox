...
{
    private static IAppLogger Logger { get; } = AppLogging.CreateLogger<AppRootPathProvider>();
    /// <inheritdoc />
    ...
    public AppRootPathProvider(IWebHostEnvironment environment)
    {
        ...
        if (isDesignTimeTool)
        {
            ...
            Logger.Info("[AppRootPathProvider] 检测到设计时工具 '{ProcessName}'。使用当前工作目录作为根目录: {RootPath}", processName, this.RootPath);
            return;
        }
        ...
        if (environment.IsDevelopment())
        {
            ...
            Logger.Info("[AppRootPathProvider] 物理应用根路径解析为: {RootPath}", this.RootPath);
            return;
        }
        ...
        while (searchDir is not null && maxDepth-- > 0)
        {
            ...
            //    这是打包后应用根目录的可靠标志。
            if (Directory.Exists(Path.Combine(searchDir.FullName, "backend")))
            {
        ...
        else
        {
            ...
            throw new DirectoryNotFoundException(
                $"无法确定打包后的应用根目录。从 '{exeDir}' 向上查找，未能找到包含 'backend' 子目录的文件夹。"
            );
        }
