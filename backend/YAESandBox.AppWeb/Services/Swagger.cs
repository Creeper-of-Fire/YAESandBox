...
{
    private static IAppLogger Logger { get; } = AppLogging.CreateLogger<SignalRDtoDocumentFilter>();
    ...
    {
        Logger.Info("正在为文档 '{ContextDocumentName}' 应用 SignalR DTO 注入。", context.DocumentName);
        ...
        Logger.Info("找到 {DtoTypesCount} 个 SignalR DTO 类型需要确保在 Schema 中:", dtoTypes.Count);
        foreach (var dtoType in dtoTypes)
        {
            Logger.Info("- {DtoTypeFullName}", dtoType.FullName);
        ...
        Logger.Info("SignalR DTO Schema 生成检查完成。");
    }
}